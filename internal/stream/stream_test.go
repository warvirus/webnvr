...
func TestDimensionsOf(t *testing.T) {
	...
	// H.265 SPS — mediacommon h265 테스트의 1920x1080 샘플 (3-5 회귀)
	sampleSPS265 := []byte{
	...
	w, h = dimensionsOf(CodecH265, sampleSPS265)
	if w != 1920 || h != 1080 {
		t.Errorf("dimensionsOf(H265) = %dx%d, want 1920x1080", w, h)
	}
	if w, h := dimensionsOf(CodecH265, nil); w != 0 || h != 0 {
		t.Errorf("H265 nil SPS 처리 실패: %dx%d", w, h)
	}
}
