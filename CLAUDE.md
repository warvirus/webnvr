...
## 11. Progress Tracking & Memory Management

**Save All Progress:** Always document and update all project progress in `MEMORY.md`. 

- **Timestamp Updates:** Ensure every new entry or change includes the exact date and time it was made.
- **Context Continuity:** Routinely reference `MEMORY.md` before starting any task to ensure seamless continuity.
- **Cross-Device Sync:** Maintain detailed logs so that work can be resumed seamlessly even when switching to a different development PC.

Prevent Recurring Issues:** Before resolving a problem, review past issues to avoid repeating the same mistakes. Take a moment to think critically and analyze the root cause from multiple angles.
