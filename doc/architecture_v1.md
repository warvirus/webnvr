...
| 인증 | 없음 (Phase 6로 연기) |
| 상정 사용처 | 주차장 출입구를 감시하는 소규모 관제에서 시작하며, **최종적으로 국제 제품 수준의 대형 관제까지 확장을 전제**한다 (§1.3 비전 참조). 현재 구현 목표는 20~500채널 |
...
### 1.3 제품 비전 · 설계 철학 (사용자 확정 — 이 문서의 최상위 제약)

> 사용자는 대형 시스템 개발 경험자이며, 초기 설계부터 **세심하고, 상세하고, 대형화를 고민하고,
> 최종적으로 국제적인 제품**(다국어 포함)을 지향한다. 이 절은 아키텍처의 모든 변경이
> 통과해야 하는 최상위 기준이며, 상세 내용은 `doc/context-notes.md` D35~D37에 있다.

**비전과 스코프**

| 축 | 기준 |
|----|------|
| 현재 구현 목표 | 서버 녹화 **20~500채널** — 단일 노드 + 단일 SQLite(WAL) 아키텍처가 이 구간의 정답 |
| 장기 확장 목표 | **100,000 ~ 10,000,000 카메라**까지 업그레이드 가능한 시스템 (D36) |
| 클라이언트 | 목록/검색에서 최대 **128채널**, 동시 라이브 뷰잉 최대 **25채널** (그리드 5×5) |
| 우선순위 | 규모 효율보다 **사건 발생 시점의 빠른 탐색** 우선 — 세그먼트 10초 고정 |

**확장 사다리 (계층 교체 지점)** — 각 단계는 "구현 교체"로 상승하며 API/프론트 계약은 불변:

| 단계 | 채널 | 인덱스 | 스토리지 |
|------|------|--------|----------|
| L1 (현재) | 20~500 | 단일 SQLite `segments` (WAL) | StoragePool → 로컬/NAS |
| L2 | ~2천 | + SumBytes 러닝 카운터 | 다중 StoragePool |
| L3 | ~1만 | SQLite 샤드(카메라 해시) + scatter-gather | Recorder Node별 로컬 NVMe |
| L4 | ~10만 | 분산 메타 인덱스 + 사전집계 | 오브젝트 스토리지(S3/MinIO) + 노드 캐시 |
| L5 | ~1,000만 | 멀티테넌트 메타 클러스터, 리전 샤딩 | 리전별 오브젝트 스토리지 + 수명주기 티어 |

**되돌릴 수 없는 설계 원칙 8 (L1에서 확정 — 새 코드 리뷰 기준)**

1. 세그먼트 = **불변 파일** + append-only 인덱스 행 (오브젝트 스토리지 이전 가능성) ✓
2. 녹화 접근은 반드시 **인덱스(`segments`) 경유** — 경로 직접 추측 금지 ✓
3. 세그먼트 식별 = (`camera_id`, `start_ts`) 시간 기반 자연키 ✓
4. 스토리지 접근은 **StoragePool 추상화 경유** ✓
5. 카메라 ID는 전역 유일 문자열, 재배정/샤딩의 기본 단위 ✓
6. API 계약과 내부 구현 분리 — 추상(카메라/시각/바이트)만 노출
7. 시간은 **UTC 저장(epoch ms) + 표시만 로케일** — `date(...,'localtime')` 사용처는 L4 전에 파라미터화 (기술부채)
8. 인덱스 선두 컬럼 확장 여부 유지 (`tenant_id` 등은 상위 단계 첫 마이그레이션)

**국제화(i18n) 준비성 6원칙 (D37)**

1. UI 문자열은 **완성형**으로 — 문장 조립(어순 파괴) 금지
2. **오류를 텍스트 매칭으로 분기 금지** — 안정적 에러 `code` 필드 사용 (현재 한글 contains 매칭 존재 → 기술부채)
3. 저장은 UTC, 표시는 로케일 — `'ko-KR'` 하드코딩은 i18n 시점에 로케일 주입으로 교체 (기술부채)
4. 표시명(카메라 이름)과 식별자(cameraID)의 분리 — 스토리지 경로에 표시명 금지 ✓
5. UTF-8 전 구간 ✓
6. 로그(개발자 대상)와 사용자 노출 문자열 구분

**태도 원칙**: 현재 단계의 코드를 복잡하게 하지 않는다. 대신 **모든 변경이 위 원칙 8+6개를 위반하지 않는지 확인**하며, 위반은 "계층 경계 위반"으로 분류해 그 시점에 수정한다. 성장 신호가 보이면 사다리의 다음 단계 구현으로 이행한다.
...
## 6ter. 영상 검색 기능 (PlaybackPage — 사용자 확정 동작 명세)

> 이 절은 녹화된 영상을 검색·재생하는 "영상 검색" 화면(§4의 PlaybackPage)의 확정 동작을
> 구현 수준으로 기술한다. 사용자가 확정한 요구사항 1-1~1-4, 2-1~2-4와 실제 녹화 시간
> 표시를 포함하며, 이후 이 화면의 모든 변경은 이 명세와 일치해야 한다. (커밋 25e2f56)

### 6ter.1 화면 구성

| 영역 | 내용 |
|------|------|
| 요약 행 | 선택 날짜 녹화 대수·총 용량, 전체 사용량, 녹화 중 대수 |
| 날짜 네비 | ◀ 이전 날 / **날짜 버튼(▾ 클릭 시 달력 오버레이 — 버튼 아래, 레이아웃 밀림 없음)** / ▶ 다음 날 + 새로고침 |
| 채널 카드 | 전 카메라(layout_order 순) — CH번호·이름·녹화 시각대(HH:MM~HH:MM)·용량·**실제 녹화 시간**·이벤트 건수, 녹화 있는 채널에 붉은 점 |
| 타임라인 | 줌/팬 가능한 시간바(아래 6ter.3), 이벤트 마커, **붉은 재생 위치 커서** |
| meta 행 | 구간 수 · **실제 녹화 시간** · 용량 · 이벤트 건수 · 재생 위치(벽시계) |
| 재생 영역 | hls.js(Chromium)/네이티브(Safari) HLS `<video>` — 카메라명 표시, 첫 구간 재생 버튼 |

### 6ter.2 데이터/API

- `GET /api/recordings/{id}?from&to` — 타임라인(ranges 병합/segments/events/usedBytes)
- `GET /api/recordings/overview?from&to` — 채널별 요약. `totalDurMs`(=`SUM(dur_ms)`, 공백 제외 실제 녹화 시간) 포함
- `GET /api/recordings/{id}/playlist.m3u8?from&to` — VOD 재생목록(세그먼트 겹침 조회, 공백 `#EXT-X-DISCONTINUITY`, `#EXT-X-PROGRAM-DATE-TIME`)
- `GET /api/recordings/{id}/seg/{segId}` — MPEG-TS 바이트(Range 206)
- `GET /api/recordings/days`, `/api/recordings/status` — 달력 마커/녹화 상태

### 6ter.3 타임라인 줌·팬·커서 (2-1, 2-2)

- **휠 줌** — timebar 위 휠: 커서 위치(포커스 시각)를 고정한 채 윈도우 폭을 `×1.25/틱` 배율로
  변경. 폭 클램프 = 15분 ~ 24시간. `WheelEvent`는 `{passive: false}`로 직접 등록(preventDefault).
- **드래그 팬** — pointer capture 기반 좌우 이동. **이동량 > 5px이면 드래그(팬), 이하면 클릭(seek)**.
- **클릭 seek** — 클릭 지점(윈도우 좌표→시각)을 포함하는 세그먼트를 찾아 그 세그먼트 **내부 오프셋부터** 재생한다
  (세그먼트 시작이 아닌 정밀 위치). 구간 외 클릭은 다음 세그먼트.
- **적응형 축** — 윈도우 폭에 따라 눈금 간격 6시간/2시간/30분/15분/5분 자동 선택, 라벨은 HH:MM.
- **붉은 커서** — `position:absolute` 수직선(머리 삼각형 포함). 위치 = 현재 벽시계를 윈도우 좌표로
  환산(`timeupdate`마다 ref 직접 갱신 — 리렌더 없음). 윈도우 밖이면 숨김.

### 6ter.4 현재 벽시계 매핑 (재생 위치의 진실)

- 플레이리스트 0초 지점의 벽시계 `playBase`(재생 시작 시 포함 세그먼트의 `start_ts`)를 기록한다.
- **현재 벽시계 = `playBase` + `video.currentTime`×1000.** 미디어 시간은 공백(DISCONTINUITY)을
  건너뛰어 누적되므로, 벽시계는 세그먼트 경계를 넘어도 정확하다. `seekTs`만으로 계산하면
  이어받기·오프셋 재생 후 위치가 틀어진다.

### 6ter.5 카메라 선택 자동 표시 (1-1~1-4)

채널 카드 클릭 = 명시적 표시 요청. 타임라인 로드가 완료되면 아래 규칙으로 자동 재생한다.
(기본 자동 선택 효과는 재생하지 않는다 — 클릭 제스처만 자동재생 대상)

| 조건 | 동작 |
|------|------|
| 1-1 시간바 미선택 | 그날 **첫 녹화 구간 시작**부터 재생 |
| 1-2 시간바 선택(seekTs 존재) | seekTs를 **포함하는 세그먼트** 찾아 내부 오프셋부터 정밀 재생 |
| 1-3 같은 시각 녹화 없음 | **이전/이후 중 절대 시간 차 최소** 세그먼트(동률은 이전)에서 재생, seekTs를 실제 시작점으로 갱신 |
| 1-4 그날 녹화 전무 | HLS 정지 + 영상 영역에 **"녹화된 영상이 없습니다"** 오버레이(채널명·날짜) + 컴팩트 메시지 박스 |

- **stale 방지** — 자동 표시 요청은 `{카메라ID, 모드}`를 ref에 기록하고, 도착한 타임라인의
  `cameraId`가 요청과 일치할 때만 실행한다(연속 클릭 시 이전 카메라 타임라인으로 재생하는 race 제거).
- **정밀 시작 구현** — `pendingOffsetRef`에 오프셋(ms)을 담고 `loadedmetadata` 1회 핸들러가
  `video.currentTime = 오프셋/1000` 적용 후 `play()`.

### 6ter.6 실시간 이어보기 (2-3)

- **60초 주기**: ①타임라인/전체 현황 재조회 ②재생 중이고 **재생이 끝에 도달했거나 끝 30초
  이내**(`ended || duration - currentTime ≤ 30s`)이면 플레이리스트를 재로딩하고 현재 벽시계에서 이어받기.
- **설계 근거** — 플레이리스트는 VOD(고정 from~to)라 hls.js가 신규 세그먼트를 자동 수집하지 못한다.
  재생 중 강제 재로딩은 끊김을 유발하므로 "끝 도달 시에만" 갱신한다. 정지 중에는 재생하지 않는다(사용자 의도 존중).
- **날짜 변경** — 재생 정지 + seekTs/줌 리셋(이전 날짜 영상이 계속 재생되는 부작용 제거).

### 6ter.7 실제 녹화 시간 (사용자 요구)

- **정의** — 벽시계 구간이 아니라 `SUM(dur_ms)`(공백 제외, 실제 촬영 시간).
- **표시 위치** — ①meta 행 "실제 녹화 시간 3시간 24분" ②줌 시 "윈도우 내 실제 녹화 시간" 병기
  ③채널 카드 "실제 녹화 2시간 10분".
- 포맷 `fmtDur`: 시간/분/초 절단 표기.

### 6ter.8 브라우저 호환

- Chromium 계열 — hls.js(번들, `enableWorker`).
- Safari — `<video src=playlist.m3u8>` 네이티브 HLS. 자동 표시·이어받기·커서 로직은 양쪽 동일
  (`loadedmetadata`/`timeupdate` 표준 이벤트 기준).

### 6ter.9 알려진 제한 (후속)

- 세그먼트 내부 오프셋 seek는 첫 세그먼트 기준이라, 재생이 공백을 건넌 뒤 임의 위치 재seek 시
  동일 방식으로 재계산된다(정확).
- 줌 상태에서 카메라/날짜 전환 시 윈도우는 유지되나, 날짜 변경 시 24시간으로 리셋된다.
- `/api/recordings/*` 무인증 — §6bis와 동일한 Phase 6.1 전 환경 한정.
...
| v1.2 (본 문서) | 2026-09-10 | **§6ter 영상 검색 기능 명세 신설** — 자동 표시(1-1~1-4), 줌/팬 타임라인(휠/드래그), 붉은 커서(벽시계=playBase+mediaTime), 실시간 이어보기(끝 도달 방식), 실제 녹화 시간(overview totalDurMs). Phase R.1~R.5 녹화/SQLite 이관 반영 |
| v1.1 (본 문서) | 2026-09-10 | **§1.3 제품 비전·설계 철학 신설** — 사용자 확정: 현재 목표 20~500채널, 장기 10만~1,000만 카메라 확장 사다리(L1~L5), 불변 설계 원칙 8, i18n 준비성 6원칙(에러 code 필드, 'ko-KR' 하드코딩은 기술부채). 녹화 §(R.1~R.5 구현), SQLite 이관, 클라이언트 128채널/동시 뷰잉 25채널 반영은 후속 개정에서 |
