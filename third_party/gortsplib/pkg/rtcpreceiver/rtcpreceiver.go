...
type RTCPReceiver struct {
	...
	// AllowSSRCChange allows the remote SSRC to change mid-session instead of
	// returning an error. Some RTSP servers (e.g. Python-based virtual cameras)
	// restart their encoder pipeline when a new client connects, which changes
	// the SSRC and would otherwise kill every existing session.
	// (webnvr patch — upstream gortsplib has no such option)
	AllowSSRCChange bool
...
func (rr *RTCPReceiver) ProcessPacket2(
	...
	if pkt.SSRC != rr.remoteSSRC {
		// webnvr patch: with AllowSSRCChange, adopt the new SSRC instead of
		// failing the whole session. Packet streams from restarted encoders
		// are still valid RTP.
		if !rr.AllowSSRCChange {
